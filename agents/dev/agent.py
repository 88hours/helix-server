...
from core.models import BugDiagnosis, CrashReport, PRResult, Project, QAResult
...
async def handle(
    ...
    # Step 2 — LLM produces a structured diagnosis (file, line, before/after).
    # For direct API providers (anthropic, openrouter) we call complete(agent="dev").
    # For goose, the dev agent runs as a CLI subprocess so we can't use it for a
    # plain JSON call — we use agent="qa" instead (always a direct API call).
    # For claude-code and opencode, diagnosis is skipped: those CLIs are capable
    # enough to discover the fix themselves.
    dev_cfg = get_agent_config("dev")
    diagnosis: Optional[BugDiagnosis] = None
    if dev_cfg.provider in ("claude-code", "opencode"):
        logger.info("skipping diagnosis — agentic provider handles discovery internally", extra={"incident_id": incident_id, "provider": dev_cfg.provider})
    else:
        diagnosis_agent = "qa" if dev_cfg.provider == "goose" else "dev"
        diagnosis_prompt = prompts.build_diagnosis(
        ...
        logger.info("dev agent calling llm for diagnosis", extra={"incident_id": incident_id, "via_agent": diagnosis_agent})
        try:
            raw = await complete(agent=diagnosis_agent, prompt=diagnosis_prompt)
            diagnosis = _parse_and_validate_diagnosis(raw, source_files, incident_id)
        except EnvironmentError as exc:
            logger.warning("diagnosis skipped — API key not available: %s", exc, extra={"incident_id": incident_id})
    await publish_tool_event(redis_client, incident_id, "dev", "llm", "complete", "success", "diagnosis")

    # Step 3 — Post the diagnosis as a GitHub Issue comment.
    if diagnosis:
        comment_body = (
            f"**Root cause:** {diagnosis.root_cause}\n\n"
            f"**Fix:** {diagnosis.fix_description}\n\n"
            f"```diff\n- {diagnosis.current_line}\n+ {diagnosis.fixed_line}\n```\n\n"
            f"*(File: `{diagnosis.file_to_edit}`, line {diagnosis.line_to_edit})*"
        )
    else:
        comment_body = "Helix is investigating this bug. A fix attempt is in progress."
    issue_comment = (
        f"**[Helix]** `{crash_report.error_type}: {crash_report.error_message}`\n\n"
        f"{comment_body}\n\n"
        f"---\n"
        f"*Incident `{incident_id}`. Implementing via TDD — a PR will follow.*"
    )
    logger.debug(
...
async def _tdd_loop(
    qa_result: QAResult,
    crash_report: CrashReport,
    diagnosis: Optional[BugDiagnosis],
    redis_client: redis.Redis,
    ...
    Args:
        ...
        diagnosis:   Validated structured fix from the diagnosis step, or None.
        redis_client: Async Redis client.
    ...
    try:
        ...
        while iteration <= MAX_ITERATIONS:
            ...
            prompt = prompts.build_tdd(
                ...
                diagnosis=diagnosis if iteration == 1 else None,
...
def _parse_and_validate_diagnosis(
    raw: str,
    source_files: dict[str, str],
    incident_id: str,
) -> Optional[BugDiagnosis]:
    """
    Parse the LLM's JSON diagnosis response and validate it against source_files.

    Validation checks:
      1. The JSON parses into a BugDiagnosis without missing fields.
      2. The file_to_edit is one of the files we actually fetched.
      3. The current_line text appears in that file's content.

    Returns the validated BugDiagnosis, or None if any check fails.
    The caller proceeds with unguided TDD on None.
    """
    try:
        data = json.loads(raw)
        diagnosis = BugDiagnosis(**data)
    except Exception as exc:
        logger.warning(
            "diagnosis parse failed — proceeding without structured hint",
            extra={"incident_id": incident_id, "error": str(exc)},
        )
        return None

    file_content = source_files.get(diagnosis.file_to_edit)
    if not file_content:
        logger.warning(
            "diagnosis references unknown file",
            extra={"incident_id": incident_id, "file": diagnosis.file_to_edit},
        )
        return None

    if diagnosis.current_line.strip() not in file_content:
        logger.warning(
            "diagnosis current_line not found in file — stale or hallucinated",
            extra={"incident_id": incident_id, "file": diagnosis.file_to_edit, "current_line": diagnosis.current_line},
        )
        return None

    logger.info(
        "diagnosis validated",
        extra={"incident_id": incident_id, "file": diagnosis.file_to_edit, "line": diagnosis.line_to_edit},
    )
    return diagnosis
