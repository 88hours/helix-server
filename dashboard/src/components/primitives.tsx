...
type BadgeTone = 'neutral' | 'crash' | 'warn' | 'ok' | 'info' | 'violet' | 'amber' | 'dim';
...
export function Severity({ level }: { level: string }) {
  const fg = level === 'high' ? 'var(--crash)' : level === 'medium' ? 'var(--warn)' : 'var(--ink-3)';
  return (
    <span className="mono" style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:11, color:'var(--ink-2)' }}>
      <span style={{
        width:6, height:6, borderRadius:'50%', background: fg,
...
export function AgentChip({ id, size = 'sm' }: { id: AgentId; size?: 'sm' | 'md' }) {
  const a = AGENTS[id];
  if (!a) return null;
...
export function ToolIcon({ tool, agent }: { tool: string; agent: AgentId }) {
  const color = AGENTS[agent]?.color ?? 'var(--ink-2)';
  const I = ({ Git: Icon.git, GitHub: Icon.github, LLM: Icon.llm, Claude: Icon.claude } as Record<string, React.FC<IconProps>>)[tool] ?? Icon.dot;
  return (
    <span style={{
      width:22, height:22, borderRadius:4,
      background: `color-mix(in oklch, ${color} 10%, transparent)`,
      border: `1px solid color-mix(in oklch, ${color} 25%, transparent)`,
      color, display:'inline-flex', alignItems:'center', justifyContent:'center', flex:'0 0 auto',
    }}>
      <I size={12}/>
    </span>
  );
...
interface ButtonProps {
  children: React.ReactNode;
  variant?: 'ghost' | 'primary' | 'accent' | 'danger' | 'subtle';
  size?: 'sm' | 'md';
  onClick?: () => void;
  style?: React.CSSProperties;
  disabled?: boolean;
...
export function Button({ children, variant = 'ghost', size = 'md', onClick, style = {}, disabled, ...rest }: ButtonProps) {
...
export function Spark({ points = [], w = 72, h = 18, color = 'var(--ink-3)' }: { points?: number[]; w?: number; h?: number; color?: string }) {
...
export function LiveDot() {
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5 }}>
      <span style={{ width:7, height:7, borderRadius:'50%', background:'var(--ok)', animation:'pulse-dot 1.6s ease-in-out infinite' }}/>
